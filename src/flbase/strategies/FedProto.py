...
from ..utils import setup_optimizer, linear_combination_state_dict, setup_seed, init_centroids
from ..loss import *
...
class FedProtoClient(Client):
    def __init__(self, criterion, trainset, testset, client_config, cid, device, param, eps, **kwargs):
        super().__init__(criterion, trainset, testset,
                         client_config, cid, device, param, eps, **kwargs)

        n_classes = len(np.unique(trainset.targets))
        self.criterion = CE_KL_LS_loss(n_classes, param, eps)
        self._initialize_model()
        self.centroids = init_centroids(trainset)
    ...
    def training(self, round, num_epochs):
        ...
        for i in range(num_epochs):
            ...
            for j, (x, y) in enumerate(self.trainloader):
                ...
                # cross entropy loss
                Cs = self.centroids.get_centroids(y)
                self.centroids.update_batch(self.model, y)
                loss = self.criterion(out_g, y, Cs)
