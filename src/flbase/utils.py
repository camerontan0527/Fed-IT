...
def init_centroids(trainset):
    train_label_list = torch.tensor(trainset.targets)
    n_classes = len(torch.unique(train_label_list))
    samples_data = []
    for Class in range(n_classes):
        if not (Class in train_label_list):
            samples_data.append(torch.tensor([]))
            continue
        idx = (train_label_list == Class).nonzero().squeeze()
        try:
            bs = len(idx)
        except Exception:
            bs = 1
        ...
        class_loader = DataLoader(trainset, batch_size = bs, sampler = sampler, pin_memory = True)
        img, tar = next(iter(class_loader))
        samples_data.append(img)
    return Centroid(n_classes, samples_data)
...
def setup_clients(Client, trainset, testset, criterion, client_config_lst, device, param, eps, **kwargs):
    """
